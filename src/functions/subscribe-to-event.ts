...
export async function subscribeToEven({ name, email, }: SubscribeToEvenParams) { 
  const result = await db.insert(subscriptions).values({
    ...
    return { // retorna como objeto, pois é mais fácil de adicionar mais propriedades futuramente no retorno

      subscriberId: subscriber.id, // pega o id dentro do objeto subscriber (subscriber.id) e armazena em subscriberId
